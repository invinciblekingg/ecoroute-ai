...
const googleAdminEmail = "tiwaryaryan24@gmail.com";
...
function mapSupabaseUserRow(row) {
  if (!row) {
    return null;
  }
...
function mapSupabaseSessionRow(row) {
  if (!row) {
    return null;
  }
...
function getManagedRole(email, provider = "local", fallbackRole = "operator") {
  const normalizedEmail = normalizeString(email);

  if (normalizedEmail === googleAdminEmail) {
    return "admin";
  }
  ...
  return fallbackRole || "operator";
}

async function getSupabaseUserByEmail(email) {
  const normalizedEmail = normalizeString(email);
  if (!normalizedEmail || !hasSupabaseStorage()) {
    return null;
  }

  const rows = await selectSupabaseRows("auth_users", {
    filters: {
  ...
  return mapSupabaseUserRow(rows[0] ?? null);
}

async function getSupabaseUserById(userId) {
  if (!userId || !hasSupabaseStorage()) {
    return null;
  }
...
async function ensureSupabaseAuthUser(user) {
  if (!user || !hasSupabaseStorage()) {
    return user;
  }

  const existing = await getSupabaseUserById(user.id) ?? await getSupabaseUserByEmail(user.email);
  const normalizedEmail = normalizeString(user.email);
  const issuedAt = nowISO();
  const role = getManagedRole(normalizedEmail, user.authProvider ?? "local", user.role ?? "operator");
  const payload = {
    ...
    created_at: existing?.createdAt ?? user.createdAt ?? issuedAt,
    updated_at: issuedAt,
  };

  if (existing) {
    const rows = await updateSupabaseRows("auth_users", payload, {
      filters: {
        id: `eq.${existing.id}`,
      },
    });

    return mapSupabaseUserRow(rows[0] ?? payload);
  }

  const created = await insertSupabaseRow("auth_users", payload);
  return mapSupabaseUserRow(created ?? payload);
}

async function pruneSupabaseExpiredSessions() {
  if (!hasSupabaseStorage()) {
    return;
  }

  const now = nowISO();
  await deleteSupabaseRows("auth_sessions", {
    filters: {
      expires_at: `lt.${now}`,
    },
  }).catch(() => false);
...
export async function authenticateUser(email, password) {
  if (hasSupabaseStorage()) {
    ...
    if (supabaseUser && supabaseUser.status !== "disabled" && supabaseUser.passwordHash && supabaseUser.passwordSalt) {
      const expectedHash = Buffer.from(supabaseUser.passwordHash, "hex");
      const candidateHash = Buffer.from(hashPassword(password, supabaseUser.passwordSalt), "hex");
      ...
      if (expectedHash.length === candidateHash.length && timingSafeEqual(expectedHash, candidateHash)) {
        return sanitizeUser({
          ...
          role: getManagedRole(
            supabaseUser.email,
            supabaseUser.authProvider ?? "local",
            supabaseUser.role ?? "operator"
          ),
        });
      }
    }
...
export async function upsertOAuthUser(profile, provider = "google") {
  if (hasSupabaseStorage()) {
    ...
    const payload = {
      id: existing?.id ?? `user-${randomUUID()}`,
      name: profile?.name || existing?.name || normalizedEmail.split("@")[0] || "EcoRoute User",
      email: normalizedEmail,
      role: getManagedRole(normalizedEmail, provider, existing?.role ?? "operator"),
      status: "active",
      auth_provider: provider,
      provider_account_id: profile?.sub ?? existing?.providerAccountId ?? null,
      avatar_url: profile?.picture ?? existing?.avatarUrl ?? null,
      password_salt: existing?.passwordSalt ?? null,
      password_hash: existing?.passwordHash ?? null,
      created_at: existing?.createdAt ?? issuedAt,
      updated_at: issuedAt,
    };

    const saved = existing
      ? (await updateSupabaseRows("auth_users", payload, {
          filters: {
            id: `eq.${existing.id}`,
          },
        }))[0] ?? payload
      : (await insertSupabaseRow("auth_users", payload)) ?? payload;

    return sanitizeUser(mapSupabaseUserRow(saved));
  }
  ...
  return mutateState(async (state) => {
    ...
    if (!user) {
      user = {
        ...
        email: normalizedEmail,
        role: getManagedRole(normalizedEmail, provider, "operator"),
        status: "active",
      ...
      user.role = getManagedRole(normalizedEmail, provider, user.role ?? "operator");
      user.status = "active";
...
export async function createUserSession(userId, metadata = {}) {
  if (hasSupabaseStorage()) {
    await pruneSupabaseExpiredSessions();

    let user = await getSupabaseUserById(userId);
    ...
    if (!user) {
      ...
      user = await ensureSupabaseAuthUser(localUser);
    }
    ...
    const savedSession = (await insertSupabaseRow("auth_sessions", payload)) ?? payload;
    const managedRole = getManagedRole(user.email, user.authProvider ?? "local", user.role ?? "operator");
    ...
    return {
      session: mapSupabaseSessionRow(savedSession),
      user: sanitizeUser({
        ...user,
        role: managedRole,
        updatedAt: issuedAt,
...
export async function getSessionById(sessionId) {
  ...
  if (hasSupabaseStorage()) {
    ...
    const rows = await selectSupabaseRows("auth_sessions", {
      filters: {
        id: `eq.${sessionId}`,
      },
      limit: 1,
    });
    const session = mapSupabaseSessionRow(rows[0] ?? null);
    ...
    const user = await getSupabaseUserById(session.userId);
    if (!user) {
      return null;
    }
    ...
    return {
      ...
      user: sanitizeUser({
        ...user,
        role: getManagedRole(user.email, user.authProvider ?? "local", user.role ?? "operator"),
      }),
    };
  }
...
export async function deleteSession(sessionId) {
  ...
  if (hasSupabaseStorage()) {
    const removed = await deleteSupabaseRows("auth_sessions", {
      filters: {
        id: `eq.${sessionId}`,
      },
    });
